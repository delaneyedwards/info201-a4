...
server <- function(input, output){
  output$linechart <- renderPlotly({
    choices_cols <- dataset %>%
      select(country, year, co2, co2_growth_prct, co2_growth_abs, consumption_co2, co2_per_capita,
             share_global_co2, co2_per_gdp, co2_per_unit_energy, cement_co2,
             coal_co2, flaring_co2, gas_co2, oil_co2, total_ghg, methane,
             nitrous_oxide)
      
    colnames(choices_cols) <- c("Country", "year", 
    ...
    plot <- ggplot(data = final_data, aes(x = year, y = .data[[input$y_input]]))+
